...
pkgs <- c("jsonlite", "dplyr", "rvest", "purrr", "ggplot2", "gganimate", "ggthemes", "lubridate", "zoo", "shiny", "stringr", "mice", "VIM", "parallel", "corrplot", "MASS", "caTools", "nnet", "randomForest", "pROC", "caret", "ggpubr", "shiny", "glmnet", "RSocrata", "ggcorrplot", "rsconnect", "pmlr")
for (pkg in pkgs) {
  pkgTest(pkg)
}
```
...
Accuracy is the highest with the ordinary logistic model. I decided to sacrifice 1% prediction accuracy in favor of a ridge-regularized logit regression because (1) my prior belief is that all covariates have some impact on the response and (2) ridge regularization handles the collinearity which inflated variance for the ordinary logit. The confusion matrix demonstrates that my model performs fairly, but needs improvement. 
...
Find my R Shiny visualization of my results here (run the rmd to the bottom): https://github.com/alj47/STA440-Individual-CS/blob/master/Austin%20Jia%20Interim%202%20FINAL%20FINAL.Rmd
