...
pkgs <- c("jsonlite", "dplyr", "rvest", "purrr", "ggplot2", "gganimate", "ggthemes", "lubridate", "zoo", "shiny", "stringr", "mice", "VIM", "parallel", "corrplot", "MASS", "caTools", "nnet", "randomForest", "pROC", "caret")
for (pkg in pkgs) {
  pkgTest(pkg)
}
```
...
#Removing Covariates
redundant_cols = c("Instruction.Rating", "Supportive.Environment.Rating", "Community.Ties.Rating", "Teachers.Rating", "Leadership.Rating", "Trust.Rating", "DBN", "Percent.HRA.Eligible", "Effective.Teaching.qr", "Expectations.Communicated.qr", "Safe.Inclusive.qr", "Meet.Goals.qr", "Decisions.Evaluated.qr", "Challenging.Curriculum.qr", "School.Assess.qr", "Teachers.Cooperation.qr", "Resource.Allocation.qr", "Teacher.Development.qr", "Review.Dates.qr", "Students.Chronically.Absent", "Percent.in.Temp.Housing")
...
We used mean/mode imputation to impute the covariates that were missing values within the threshold of 0% to 10%. This is because the results of doing mean imputation in covariates with only a small amount of missing values will not differ significantly from doing MICE on all covariates, which would have taken a lot more time. In the graph below, we can see that after doing mean imputation on the covariates missing small amounts of data, only the covariates missing significant amounts of data are left. We will use MICE to perform imputation with these covariates.
...
data3 <- parlmice(data2, m=2, printFlag=TRUE, maxit = 2,
...
g5 = ggplot(data2,aes(x=Percent.Asian,fill=factor(Student.Achievement.Rating)))+geom_density(alpha=0.25)+
  geom_vline(aes(xintercept=mean(Percent.Asian[Student.Achievement.Rating=="Not Meeting Target"],na.rm=T)),color="red",linetype="dashed",lwd=1)+
  geom_vline(aes(xintercept=mean(Percent.Asian[Student.Achievement.Rating=="Meeting Target"],na.rm=T)),color="green",linetype="dashed",lwd=1)+
  scale_x_continuous(breaks = seq(8,15,1))+
  xlab(label = "Percent Asian")+
...
g6 = ggplot(data2,aes(x=Percent.Black,fill=factor(Student.Achievement.Rating)))+geom_density(alpha=0.25)+
...
g7 = ggplot(data2,aes(x=Percent.White,fill=factor(Student.Achievement.Rating)))+geom_density(alpha=0.25)+
...
#Faculty Experience
g8 = ggplot(data2,aes(x=Principal.Experience.yrs,fill=factor(Student.Achievement.Rating)))+geom_density(alpha=0.25)+
...
g9 = ggplot(data2,aes(x=Teachers.3yrs.Experience,fill=factor(Student.Achievement.Rating)))+geom_density(alpha=0.25)+
...
ggarrange(p_year, p_race, p_county, p_map, ncol=2, nrow=2)
```
...
pred.test.glm <- predict(glm.fit, testset)
pred.test.glm
pred.test.rf <- predict(rf.fit, testset)
pred.test.rf
```

### Model Selection

Misclassification error is 32% for logistic regression and 42% for random forest. The logistic regression is also easier to interpret. For these reasons, we move ahead with the logistic regression. 
...
- How to scrape data?
- MICE not working?
- MICE before or after combining factor levels
- Confusion matrix outputs really weird
